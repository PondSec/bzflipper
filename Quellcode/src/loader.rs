//! PursePilot-loader
//!
//! A lightweight auto-updater for PursePilot.
...
//!
//! Usage: just run `PursePilot-loader` instead of `purse_pilot` directly.
...
fn platform_asset_name() -> Option<&'static str> {
    if cfg!(all(target_os = "linux", target_arch = "x86_64")) {
        Some("purse_pilot-linux-x86_64")
    } else if cfg!(all(target_os = "macos", target_arch = "x86_64")) {
        Some("purse_pilot-macos-x86_64")
    } else if cfg!(all(target_os = "macos", target_arch = "aarch64")) {
        Some("purse_pilot-macos-arm64")
    } else if cfg!(all(target_os = "windows", target_arch = "x86_64")) {
        Some("purse_pilot-windows-x86_64.exe")
    } else {
        None
    }
}
...
fn check_and_update() -> anyhow::Result<()> {
    ...
    let client = reqwest::blocking::Client::builder()
        .user_agent("PursePilot-loader/1.0")
    ...
    if local_version.as_deref() == Some(latest_tag.as_str()) {
        println!("[Loader] Already up-to-date. Launching PursePilot…");
        return Ok(());
    }
...
fn download_and_replace(
    ...
    {
        let tmp = exe_dir.join("purse_pilot_new.exe");
        fs::write(&tmp, bytes)?;
    ...
    {
        let tmp = exe_dir.join("purse_pilot_new");
        fs::write(&tmp, bytes)?;
