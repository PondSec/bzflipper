...
pub struct BotClient {
    ...
    /// Set when Hypixel reports materials/items in `/pickupstash`.
    stashed_items_present: Arc<AtomicBool>,
...
impl BotClient {
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            stashed_items_present: Arc::new(AtomicBool::new(false)),
    ...
    pub async fn connect(
        ...
        let handler_state = BotClientState {
            ...
            stashed_items_present: self.stashed_items_present.clone(),
    ...
    /// Returns true if Hypixel reported materials/items in `/pickupstash`.
    pub fn has_stashed_items(&self) -> bool {
        self.stashed_items_present.load(Ordering::Relaxed)
    }

    /// Clear the stash flag after a pickup attempt. If Hypixel reminds us again,
    /// the chat handler sets it back to true.
    pub fn clear_stashed_items_flag(&self) {
        self.stashed_items_present.store(false, Ordering::Relaxed);
    }
...
pub struct BotClientState {
    ...
    /// Set when Hypixel reports materials/items in `/pickupstash`.
    pub stashed_items_present: Arc<AtomicBool>,
    ...
    match event {
        ...
        Event::Chat(chat) => {
            ...
            if clean_message.contains("stashed away") {
                state.stashed_items_present.store(true, Ordering::Relaxed);
            ...
            if clean_message.contains("don't have any items stashed")
                || clean_message.contains("don't have anything stashed")
                || clean_message.contains("no items stashed")
            {
                state.stashed_items_present.store(false, Ordering::Relaxed);
            }
            ...
            if window_title.contains("Bazaar") && !is_bazaar_orders_window_title(window_title) {
                ...
                if !name_key.is_empty() && state.manage_orders_processed.read().contains(&name_key)
                {
                    ...
                    *state.bot_state.write() = BotState::Idle;
                    ...
                    let targeted_cancel = target_item.is_some();
                    let too_young_to_cancel =
                        !targeted_cancel && is_order_below_min_cancel_age(&order_identity);
                    ...
                    let cancel_due_to_age = targeted_cancel
                        || (!cancel_open
                            && should_cancel_open_order_due_to_age(
                                order_identity,
...
async fn run_startup_workflow(
    ...
    // Step 2/4: Bazaar order management — collect filled orders and reconcile
    // open ones without cancelling profitable orders on every restart. Targeted
    // reprice logic handles stale/outbid orders later.
    let cancel_open = false;
    let mode_str = if cancel_open {
        "cancel + collect"
    } else {
        "collect-only"
