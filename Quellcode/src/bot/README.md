...
This module implements the core Azalea bot client for PursePilot (Bazaar Auction Flipper).
...
The implementation preserves all critical logic from `/tmp/PursePilot/src/BAF.ts`:
...
```rust
use purse_pilot::bot::client::BotClient;
...
- **FastWindowClick**: `/tmp/PursePilot/src/fastWindowClick.ts`
- **Bazaar Handler**: `/tmp/PursePilot/src/bazaarFlipHandler.ts`
